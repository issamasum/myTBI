...
Current tool packages live under `tools/`. For example, `tools/checkin/` keeps the Check-in feature's tables, repository, service, and models.
...
```python
def get_course_roster(self, subject: User, event: Event) -> list[Visitor]: ...
```
